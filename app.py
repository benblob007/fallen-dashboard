...
async def tournaments_page(request: Request):
    ...
    try:
        ...
        c["season"] = await db.get_active_tournament_season()
    except: c.update(active=[], upcoming=[], completed=[], season=None)
...
async def tournament_detail_public(request: Request, tid: int):
    ...
    try:
        ...
        if c.get("user") and c["tournament"]:
            ...
            c["is_checked_in"] = any(p["user_id"] == c["user"]["id"] and p.get("checked_in") for p in (c["tournament"].get("participants") or []))
        else: c["is_joined"] = False; c["is_checked_in"] = False
    except: c["tournament"] = None; c["is_joined"] = False; c["is_checked_in"] = False
...
@app.post("/tournament/{tid}/checkin")
async def tournament_checkin(request: Request, tid: int):
    user = auth.get_session(request)
    if not user: return RedirectResponse("/auth/login")
    await db.checkin_participant(tid, user["id"])
    return RedirectResponse(f"/tournament/{tid}?checked_in=1", status_code=303)

# ================================================
# PUBLIC RANKINGS (TOP 10)
# ================================================
@app.get("/rankings", response_class=HTMLResponse)
async def rankings_page(request: Request):
    c = await _ctx(request)
    fmt = request.query_params.get("format", "1v1")
    try:
        c["rankings"] = await db.get_top_rankings(fmt=fmt, limit=10)
        c["format"] = fmt
        c["season"] = await db.get_active_tournament_season()
        c["tier_list"] = db.RANKING_TIERS
    except: c.update(rankings=[], format=fmt, season=None, tier_list=[])
    return templates.TemplateResponse("rankings.html", c)

# ================================================
# PUBLIC ALLIES
# ================================================
@app.get("/allies", response_class=HTMLResponse)
async def allies_page(request: Request):
    c = await _ctx(request)
    try:
        all_allies = await db.get_allies(visibility="public")
        c["active"] = [a for a in all_allies if a["status"] == "active"]
        c["paused"] = [a for a in all_allies if a["status"] == "paused"]
    except: c.update(active=[], paused=[])
    return templates.TemplateResponse("allies.html", c)

@app.get("/allies/apply", response_class=HTMLResponse)
async def ally_apply_page(request: Request):
    c = await _ctx(request)
    c["success"] = request.query_params.get("success", "")
    c["error"] = request.query_params.get("error", "")
    return templates.TemplateResponse("ally_apply.html", c)

@app.post("/allies/apply")
async def ally_apply_submit(r: Request, server_name: str = Form(...), server_size: int = Form(0),
        invite_link: str = Form(""), reason: str = Form(""), offering: str = Form(""),
        contact_discord_id: str = Form(""), contact_name: str = Form("")):
    if len(server_name) < 2: return RedirectResponse("/allies/apply?error=name_too_short", status_code=303)
    if len(reason) < 10: return RedirectResponse("/allies/apply?error=reason_too_short", status_code=303)
    try:
        app = await db.create_ally_application(server_name, server_size, invite_link, reason, offering, contact_discord_id, contact_name)
        if app:
            # Notify staff
            await db.add_notification(0, "ally_application", f"New ally application: {server_name}")
        return RedirectResponse("/allies/apply?success=submitted", status_code=303)
    except:
        return RedirectResponse("/allies/apply?error=failed", status_code=303)
...
async def staff_tournaments(request: Request):
    ...
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    try:
        c["tournaments"] = await db.get_tournaments(limit=30)
        c["season"] = await db.get_active_tournament_season()
        c["disputes"] = await db.get_tournament_disputes(status="open", limit=5)
    except: c.update(tournaments=[], season=None, disputes=[])
    c["success"] = request.query_params.get("success", "")
...
async def create_tournament(r: Request, title: str = Form(...), bracket_size: int = Form(8),
        match_format: str = Form("1v1"), is_ranked: str = Form("off"),
        best_of: int = Form(1), game_mode: str = Form("TSB"),
        description: str = Form(""), entry_type: str = Form("open"),
        entry_requirement: str = Form(""), entry_fee: int = Form(0),
        prize_pool: str = Form(""), prize_coins: int = Form(0), prize_robux: int = Form(0),
        match_rules: str = Form(""), screenshot_required: str = Form("off"),
        checkin_required: str = Form("off"), checkin_minutes: int = Form(30),
        timeout_minutes: int = Form(15), no_show_penalty: int = Form(0)):
    c = await _ctx(r)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    s = c["user"]; season = await db.get_active_tournament_season()
    tid = await db.create_tournament(
        title=title, description=description, bracket_size=bracket_size,
        match_format=match_format, is_ranked=(is_ranked == "on"),
        best_of=best_of, game_mode=game_mode, entry_type=entry_type,
        entry_requirement=entry_requirement, entry_fee=entry_fee,
        prize_pool=prize_pool, prize_coins=prize_coins, prize_robux=prize_robux,
        match_rules=match_rules, screenshot_required=(screenshot_required == "on"),
        checkin_required=(checkin_required == "on"), checkin_minutes=checkin_minutes,
        timeout_minutes=timeout_minutes, no_show_penalty=no_show_penalty,
        season_id=season["id"] if season else None, created_by=s["id"])
    if tid: await db.add_audit(s["id"], s.get("username","?"), "created_tournament", details=f"#{tid}: {title}", ip=_get_ip(r))
    return RedirectResponse("/staff/tournaments?success=created", status_code=303)
...
async def staff_tournament_detail(request: Request, tid: int):
    ...
    try:
        c["tournament"] = await db.get_tournament(tid)
        c["disputes"] = await db.get_tournament_disputes(tid=tid, limit=20)
    except: c["tournament"] = None; c["disputes"] = []
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/tournament_detail.html", c)

@app.post("/staff/tournament/{tid}/edit")
async def edit_tournament(r: Request, tid: int, title: str = Form(...), description: str = Form(""),
        match_format: str = Form("1v1"), is_ranked: str = Form("off"),
        best_of: int = Form(1), entry_fee: int = Form(0),
        prize_pool: str = Form(""), prize_coins: int = Form(0), prize_robux: int = Form(0),
        match_rules: str = Form(""), bracket_size: int = Form(8)):
    c = await _ctx(r)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.update_tournament(tid, title=title, description=description,
        match_format=match_format, is_ranked=(is_ranked == "on"), best_of=best_of,
        entry_fee=entry_fee, prize_pool=prize_pool, prize_coins=prize_coins,
        prize_robux=prize_robux, match_rules=match_rules, bracket_size=bracket_size)
    return RedirectResponse(f"/staff/tournament/{tid}?success=updated", status_code=303)
...
async def update_tournament_status(r: Request, tid: int, status: str = Form(...)):
    ...
    # Award prizes on completion
    if status == "completed":
        t = await db.get_tournament(tid)
        if t and t.get("prize_coins") and t.get("bracket"):
            bracket = t["bracket"]
            if isinstance(bracket, str): bracket = json.loads(bracket)
            finals = (bracket.get("rounds") or [{}])[-1]
            if finals.get("matches") and finals["matches"][0].get("winner"):
                winner_id = finals["matches"][0]["winner"]
                await dispatch_action("add_coins", winner_id, c["user"]["id"], c["user"].get("username","?"),
                    {"amount": t["prize_coins"], "reason": f"Tournament #{tid} winner prize"})
...
# Staff Rankings Management
@app.get("/staff/rankings", response_class=HTMLResponse)
async def staff_rankings(request: Request):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    fmt = request.query_params.get("format", "1v1")
    try:
        c["rankings"] = await db.get_top_rankings(fmt=fmt, limit=25)
        c["format"] = fmt; c["season"] = await db.get_active_tournament_season()
        c["seasons"] = await db.get_tournament_seasons()
    except: c.update(rankings=[], format=fmt, season=None, seasons=[])
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/rankings.html", c)

@app.post("/staff/rankings/adjust")
async def staff_ranking_adjust(r: Request, user_id: int = Form(...), amount: int = Form(0),
        reason: str = Form(""), fmt: str = Form("1v1")):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    season = await db.get_active_tournament_season()
    await db.adjust_ranking(user_id, fmt, amount, reason, s["id"], season["id"] if season else 0)
    await db.add_audit(s["id"], s.get("username","?"), "ranking_adjust", details=f"User {user_id}: {amount:+d} ({reason})", ip=_get_ip(r))
    return RedirectResponse(f"/staff/rankings?format={fmt}&success=adjusted", status_code=303)

@app.post("/staff/rankings/push_embed")
async def staff_rankings_push(r: Request, fmt: str = Form("1v1"), channel_id: str = Form("")):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    rankings = await db.get_top_rankings(fmt=fmt, limit=10)
    await dispatch_action("top10_embed", 0, s["id"], s.get("username","?"), {
        "format": fmt, "channel_id": int(channel_id) if channel_id.isdigit() else 0,
        "rankings": [{"rank": i+1, "name": r["roblox_username"], "points": r["points"],
            "tier": r["tier"], "wins": r["wins"], "losses": r["losses"],
            "avatar_url": r.get("avatar_url",""), "clan_rank": r.get("clan_rank","")} for i, r in enumerate(rankings)]
    })
    return RedirectResponse(f"/staff/rankings?format={fmt}&success=embed_pushed", status_code=303)

@app.post("/staff/seasons/create")
async def create_season(r: Request, name: str = Form(...), rewards: str = Form(""), champion_role: str = Form("")):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.create_tournament_season(name, rewards, champion_role, s["id"])
    return RedirectResponse("/staff/rankings?success=season_created", status_code=303)

@app.post("/staff/seasons/{sid}/end")
async def end_season(r: Request, sid: int):
    c = await _ctx(r)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.end_tournament_season(sid)
    return RedirectResponse("/staff/rankings?success=season_ended", status_code=303)

# Tournament Disputes
@app.get("/staff/tournament-disputes", response_class=HTMLResponse)
async def staff_tournament_disputes(request: Request):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    try: c["disputes"] = await db.get_tournament_disputes(limit=50)
    except: c["disputes"] = []
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/tournament_disputes.html", c)

@app.post("/staff/tournament-disputes/{did}/resolve")
async def resolve_tournament_dispute(r: Request, did: int, resolution: str = Form(""), winner_id: int = Form(0)):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.resolve_tournament_dispute(did, resolution, winner_id, s["id"], s.get("username","?"))
    return RedirectResponse("/staff/tournament-disputes?success=resolved", status_code=303)
...
async def tournament_match_result(r: Request, tid: int, round_num: int = Form(...), match_id: int = Form(...), winner_id: int = Form(...), score: str = Form(""), p1_score: int = Form(0), p2_score: int = Form(0)):
    c = await _ctx(r)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    ok = await db.update_match_result(tid, round_num, match_id, winner_id, score, p1_score, p2_score)
...
# ================================================
# STAFF ALLIES MANAGEMENT
# ================================================
@app.get("/staff/allies", response_class=HTMLResponse)
async def staff_allies(request: Request):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    try:
        c["allies"] = await db.get_allies()
        c["stats"] = await db.get_ally_stats()
        c["applications"] = await db.get_ally_applications(status="pending")
    except: c.update(allies=[], stats={"total":0,"active":0,"paused":0,"removed":0}, applications=[])
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/allies.html", c)

@app.post("/staff/allies/add")
async def staff_allies_add(r: Request, server_name: str = Form(...), description: str = Form(""),
        invite_link: str = Form(""), ally_type: str = Form("friendly"), tier: str = Form("bronze"),
        server_icon_url: str = Form(""), server_size: int = Form(0), contact_discord_id: int = Form(0),
        notes: str = Form(""), visibility: str = Form("public")):
    c = await _ctx(r)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    s = c["user"]
    ally = await db.create_ally(server_name, description, invite_link, ally_type, tier, server_icon_url, server_size, contact_discord_id, notes, s["id"])
    if visibility == "internal" and ally:
        await db.update_ally(ally["id"], visibility="internal")
    await db.add_audit(s["id"], s.get("username","?"), "add_ally", details=f"'{server_name}' ({ally_type}, {tier})", ip=_get_ip(r))
    return RedirectResponse("/staff/allies?success=added", status_code=303)

@app.get("/staff/allies/{aid}", response_class=HTMLResponse)
async def staff_ally_detail(request: Request, aid: int):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    c["ally"] = await db.get_ally(aid)
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/ally_detail.html", c)

@app.post("/staff/allies/{aid}/edit")
async def staff_ally_edit(r: Request, aid: int, server_name: str = Form(...), description: str = Form(""),
        invite_link: str = Form(""), ally_type: str = Form("friendly"), tier: str = Form("bronze"),
        server_icon_url: str = Form(""), server_size: int = Form(0), contact_discord_id: int = Form(0),
        notes: str = Form(""), visibility: str = Form("public"), display_order: int = Form(0)):
    c = await _ctx(r)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.update_ally(aid, server_name=server_name, description=description, invite_link=invite_link,
        ally_type=ally_type, tier=tier, server_icon_url=server_icon_url, server_size=server_size,
        contact_discord_id=contact_discord_id, notes=notes, visibility=visibility, display_order=display_order)
    await db.add_audit(c["user"]["id"], c["user"].get("username","?"), "edit_ally", details=f"ally #{aid}: {server_name}", ip=_get_ip(r))
    return RedirectResponse(f"/staff/allies/{aid}?success=updated", status_code=303)

@app.post("/staff/allies/{aid}/pause")
async def staff_ally_pause(r: Request, aid: int):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.pause_ally(aid)
    await db.add_audit(s["id"], s.get("username","?"), "pause_ally", details=f"ally #{aid}", ip=_get_ip(r))
    return RedirectResponse(f"/staff/allies/{aid}?success=paused", status_code=303)

@app.post("/staff/allies/{aid}/remove")
async def staff_ally_remove(r: Request, aid: int):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.remove_ally(aid)
    await db.add_audit(s["id"], s.get("username","?"), "remove_ally", details=f"ally #{aid}", ip=_get_ip(r))
    return RedirectResponse(f"/staff/allies/{aid}?success=removed", status_code=303)

@app.post("/staff/allies/{aid}/reactivate")
async def staff_ally_reactivate(r: Request, aid: int):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    await db.reactivate_ally(aid)
    await db.add_audit(s["id"], s.get("username","?"), "reactivate_ally", details=f"ally #{aid}", ip=_get_ip(r))
    return RedirectResponse(f"/staff/allies/{aid}?success=reactivated", status_code=303)

@app.post("/staff/allies/push_embed")
async def staff_allies_push_embed(r: Request, channel_id: str = Form("")):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    allies = await db.get_allies(status="active", visibility="public")
    await dispatch_action("allies_embed", 0, s["id"], s.get("username","?"), {
        "allies": [{"name": a["server_name"], "description": a["description"], "invite": a["invite_link"],
            "type": a["ally_type"], "tier": a["tier"], "icon": a.get("server_icon_url",""),
            "allied_at": a["allied_at"].isoformat() if a.get("allied_at") else ""} for a in allies],
        "channel_id": int(channel_id) if channel_id.isdigit() else 0
    })
    await db.add_audit(s["id"], s.get("username","?"), "push_allies_embed", ip=_get_ip(r))
    return RedirectResponse("/staff/allies?success=embed_pushed", status_code=303)

# Staff Ally Applications
@app.get("/staff/allies/applications", response_class=HTMLResponse)
async def staff_ally_applications(request: Request):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    try: c["applications"] = await db.get_ally_applications(limit=50)
    except: c["applications"] = []
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/ally_applications.html", c)

@app.post("/staff/allies/applications/{app_id}/approve")
async def staff_ally_app_approve(r: Request, app_id: int, ally_type: str = Form("friendly"), tier: str = Form("bronze"), notes: str = Form("")):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    ally = await db.approve_ally_application(app_id, s["id"], s.get("username","?"), ally_type, tier, notes)
    if ally:
        # Notify via bot DM
        app = await db.get_ally_application(app_id)
        if app and app.get("contact_discord_id"):
            try:
                await dispatch_action("ally_app_notify", int(app["contact_discord_id"]), s["id"], s.get("username","?"),
                    {"status": "approved", "server_name": app["server_name"], "notes": notes})
            except: pass
        await db.add_audit(s["id"], s.get("username","?"), "approve_ally_app", details=f"app #{app_id}", ip=_get_ip(r))
    return RedirectResponse("/staff/allies/applications?success=approved", status_code=303)

@app.post("/staff/allies/applications/{app_id}/deny")
async def staff_ally_app_deny(r: Request, app_id: int, notes: str = Form("")):
    c = await _ctx(r); s = c["user"]
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    app = await db.review_ally_application(app_id, "denied", s["id"], s.get("username","?"), notes)
    if app and app.get("contact_discord_id"):
        try:
            await dispatch_action("ally_app_notify", int(app["contact_discord_id"]), s["id"], s.get("username","?"),
                {"status": "denied", "server_name": app["server_name"], "notes": notes})
        except: pass
    await db.add_audit(s["id"], s.get("username","?"), "deny_ally_app", details=f"app #{app_id}", ip=_get_ip(r))
    return RedirectResponse("/staff/allies/applications?success=denied", status_code=303)
