...
async def callback(request: Request, code: str = None, error: str = None):
    ...
    is_staff = auth.check_is_staff(uid_int, role_ids); permission_tier = 0; section_perms = {}
...
async def apply_submit(request: Request, position_id: int = Form(...), answers: str = Form(...)):
    ...
    if ok:
        ...
        await db.add_notification(None, "application", "New Application", f"{user.get('username','?')} applied", "/staff/members", is_global=True)
...
async def customize_page(request: Request):
    ...
    uid = c["user"]["id"]
    try:
        c["profile"] = await db.get_user(uid); c["backgrounds"] = LEVEL_CARD_BACKGROUNDS
...
async def customize_set_bg(request: Request, bg_key: str = Form(...)):
    ...
    await db.set_user_bg(user["id"], bg_key)
...
async def staff_dashboard(request: Request):
    ...
    except: c.update(stats={}, recent_warnings=[], open_positions=[], pending_apps=[], recent_audit=[], guardian={}, mod_stats={}, bot_status=None, current_season=None)
    ip = _get_ip(request)
...
async def staff_action_warn(r: Request, user_id: int, category: str = Form(...), reason: str = Form("")):
    ...
    await db.create_case(user_id, "warning", category, "high" if cat_info.get("instant_ban") else "medium", reason, cat_info.get("points", 0), s["id"], s.get("username","?"))
    await db.add_audit(s["id"], s.get("username","?"), "queued_warn", target_id=user_id, details=f"{category}: {reason[:100]}", ip=ip)
...
async def staff_action_promote(r: Request, user_id: int, new_rank: str = Form(...), reason: str = Form("")):
    ...
    await db.queue_action("set_rank", user_id, s["id"], s.get("username","?"), {"rank": new_rank, "reason": reason})
...
async def staff_activity(request: Request):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    try: c["activity"] = await db.get_activity_stats()
    except: c["activity"] = {}
...
async def staff_embeds(request: Request, category: str = ""):
    ...
    try:
        ...
        c["categories"] = await db.get_embed_categories()
...
async def new_embed(request: Request):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    c["embed"] = None; c["categories"] = await db.get_embed_categories()
...
async def edit_embed(request: Request, eid: int):
    c = await _ctx(request)
    if _require_staff(c): return RedirectResponse("/?error=unauthorized")
    try: c["embed"] = await db.get_embed(eid); c["categories"] = await db.get_embed_categories()
    except: c["embed"] = None; c["categories"] = []
    c["success"] = request.query_params.get("success", "")
    return templates.TemplateResponse("staff/embed_editor.html", c)
...
async def push_embed(r: Request, eid: int, channel_id: str = Form(...)):
    ...
    await db.queue_action("push_embed", 0, c["user"]["id"], c["user"].get("username","?"), {"embed_id": eid, "channel_id": cid})
