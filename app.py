...
def **init**(self, app, max_requests=60, window=60):
super().**init**(app); self.max_requests = max_requests; self.window = window; self.requests = defaultdict(list)
...
print(f”[ERROR] {type(exc).**name**}: {exc}”)
try:
return templates.TemplateResponse(“error.html”, {“request”:r,“user”:None,“is_staff”:False,“error_code”:500,“error_title”:“Server Error”,“error_msg”:str(exc)[:200]}, status_code=500)
except:
...
c = _ctx(r)
if *require_tier(c, 2): return RedirectResponse(”/?error=unauthorized”)
s = c[“user”]
await db.update_tournament_status(tid, status)
await db.add_audit(s[“id”], s.get(“username”,”?”), f”tournament*{status}”, details=f”Tournament #{tid}”)
