...
class DashboardDB:
    ...
    async def get_tournaments(self, status=None, limit=20):
        if status: return await self._qall("SELECT * FROM tournaments WHERE status=$1 ORDER BY created_at DESC LIMIT $2", status, limit)
        return await self._qall("SELECT * FROM tournaments ORDER BY created_at DESC LIMIT $1", limit)
    async def get_tournament(self, tid):
        t = await self._qone("SELECT * FROM tournaments WHERE id=$1", tid)
        if t:
            t["participants"] = await self._qall("SELECT * FROM tournament_participants WHERE tournament_id=$1 ORDER BY seed ASC", tid)
            # Enrich with names
            users = await self._users()
    ...
    async def create_tournament(self, title, bracket_size, entry_req, entry_fee, prize_pool, match_rules, created_by):
        try:
            async with self.pool.acquire() as c:
                row = await c.fetchrow("INSERT INTO tournaments (title, bracket_size, entry_requirement, entry_fee, prize_pool, match_rules, created_by) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id",
    ...
    async def update_tournament_status(self, tid, status):
        async with self.pool.acquire() as c:
            if status == "active": await c.execute("UPDATE tournaments SET status=$1, started_at=NOW() WHERE id=$2", status, tid)
            elif status == "completed": await c.execute("UPDATE tournaments SET status=$1, completed_at=NOW() WHERE id=$2", status, tid)
            else: await c.execute("UPDATE tournaments SET status=$1 WHERE id=$2", status, tid)
    async def add_tournament_participant(self, tid, user_id, seed=0):
        try:
            async with self.pool.acquire() as c:
                await c.execute("INSERT INTO tournament_participants (tournament_id, user_id, seed) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING", tid, user_id, seed)
        except Exception as e: print(f"[TOURNAMENT] Add participant error: {e}")
    async def disqualify_participant(self, tid, user_id):
        async with self.pool.acquire() as c:
            await c.execute("UPDATE tournament_participants SET status='disqualified', eliminated_at=NOW() WHERE tournament_id=$1 AND user_id=$2", tid, user_id)
    async def update_bracket(self, tid, bracket_data):
        async with self.pool.acquire() as c:
            await c.execute("UPDATE tournaments SET bracket=$1 WHERE id=$2", json.dumps(bracket_data), tid)
