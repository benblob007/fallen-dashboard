"""
THE FALLEN - Dashboard Database (v5 - Complete Feature Set)
All data: json blobs + 20 relational tables for full clan management.
"""
import os, json, asyncpg, datetime, hashlib
...
class DashboardDB:
    ...
    async def connect(self):
        ...
        print("Dashboard DB connected (v5 - full features)")
    ...
    async def get_economy_stats(self):
        ...
        return {
            ...
            "richest": [{"user_id": uid, "name": users.get(str(uid), {}).get("roblox_username", "Unknown"), "coins": c} for uid, c in coins[:10]],
        }
    ...
    async def get_open_positions(self):
        return await self._qall("SELECT * FROM recruitment_positions WHERE status='open' ORDER BY created_at DESC") if self.pool else []
