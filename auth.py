...
async def get_user_guild_permissions(access_token: str, guild_id: str) -> Dict:
    """
    Fetch the user's guild list to get their computed permissions and owner status.
    Returns {"is_owner": bool, "permissions": int, "tier": int}
    """
    headers = {"Authorization": f"Bearer {access_token}"}
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code != 200:
            print(f"[AUTH] ❌ Guilds fetch failed: {resp.status_code}")
            return {"is_owner": False, "permissions": 0, "tier": 0}
        ...
        for g in guilds:
            if str(g.get("id")) == str(guild_id):
                ...
                # Auto-detect tier from Discord permissions
                tier = 0
                if is_owner:
                    tier = 3
                    print(f"[AUTH] ✅ User is SERVER OWNER → tier 3")
                elif perms & PERM_ADMINISTRATOR:
                    tier = 3
                    print(f"[AUTH] ✅ User has ADMINISTRATOR permission → tier 3")
                elif (perms & PERM_MANAGE_GUILD) or (perms & PERM_KICK_MEMBERS and perms & PERM_BAN_MEMBERS):
                    tier = 2
                    print(f"[AUTH] ✅ User has MANAGE_GUILD or KICK+BAN → tier 2")
                elif perms & PERM_MODERATE_MEMBERS or perms & PERM_MANAGE_ROLES or perms & PERM_KICK_MEMBERS:
                    tier = 1
                    print(f"[AUTH] ✅ User has MODERATE_MEMBERS or MANAGE_ROLES → tier 1")
                else:
                    print(f"[AUTH] User permissions: {perms} (no staff perms detected)")
                
                return {"is_owner": is_owner, "permissions": perms, "tier": tier}
        
        print(f"[AUTH] ⚠️ Guild {guild_id} not found in user's guild list")
        return {"is_owner": False, "permissions": 0, "tier": 0}
